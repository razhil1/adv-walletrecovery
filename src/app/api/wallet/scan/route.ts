...
export async function POST(request: NextRequest) {
  try {
    ...
    // Auto-select batch size based on mode (smaller to avoid OOM)
    const bs = batchSize || (mode === 'fast' ? 100 : mode === 'balanced' ? 50 : 25);

    const job = startScanJob(wc, filteredChains, mode, bs, balanceCheckPercent);
